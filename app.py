...
Based on Random Forest Model and SHAP Explainability Technology
"""
...
def load_model():
    ...
    try:
        model_path = 'random_forest_optimized.pkl'
        if os.path.exists(model_path):
            model_data = joblib.load(model_path)
            return model_data['model'], model_data['scaler'], model_data['training_info']['feature_names']
        else:
            st.error("Model file not found! Please ensure random_forest_optimized.pkl exists.")
...
def interpret_risk_level(probability):
    ...
    if probability >= 0.7:
        return "High Risk", "red", "🔴", "Recommend immediate ICU transfer preparation and intensive monitoring"
    elif probability >= 0.5:
        return "Medium-High Risk", "darkorange", "🟠", "Recommend ICU physician consultation and transfer preparation"
    elif probability >= 0.3:
        return "Medium Risk", "orange", "🟡", "Recommend enhanced monitoring and close observation of condition changes"
    else:
...
def main():
    ...
    <div style="background-color: #f0f8ff; padding: 1.5rem; border-radius: 0.5rem; margin: 1rem 0;">
        ...
        <p>• <strong>Intelligent Prediction</strong>: Based on Random Forest machine learning algorithm, accurately predicts ICU transfer risk</p>
    ...
    if model is None or feature_config is None:
        ...
        if predict_button:
            ...
            if risk_level == "High Risk":
                ...
                <div class="risk-medium">
                    ...
                    <p><strong>Recommended Actions</strong>: {recommendation}</p>
                    <p><strong>Key Points</strong>: Enhance vital sign monitoring and prepare for potential transfer</p>
                </div>
                """
            else:
                risk_explanation = f"""
            ...
            try:
                ...
                try:
                    ...
                    # Create SHAP explanation object (reference step9 lines 345-350)
                    ...
                    try:
                        ...
                        # Create waterfall plot explanation object
                        exp_case = shap.Explanation(
                            values=shap_values_positive[0],
                            base_values=explainer.expected_value[1] if hasattr(explainer.expected_value, '__len__') else explainer.expected_value,
                            data=input_data_df.iloc[0],
    ...
    with st.expander("📖 User Guide and Important Notes"):
        ...
        - **🔴 High Risk (≥70%)**: Strongly recommend preparing for ICU transfer
        - **🟠 Medium-High Risk (50-70%)**: Recommend ICU physician consultation and assessment  
        - **🟡 Medium Risk (30-50%)**: Enhanced monitoring and close observation
        - **🟢 Low Risk (<30%)**: Continue routine ward treatment
        
        ### 📊 Risk Distribution Explanation
        - **Medium risk patients are more common - this is normal**: Reflects the actual situation of hospitalized pancreatitis patients
        - **Model adopts conservative strategy**: Better to observe more to avoid missing high-risk patients  
        - **Combined with SHAP analysis**: Focus on risk factors with the greatest impact
    ...
    <div style="text-align: center; color: #666; padding: 1rem;">
        <p>🏥 Pancreatitis ICU Transfer Risk Prediction System | Based on Random Forest + SHAP Technology</p>
